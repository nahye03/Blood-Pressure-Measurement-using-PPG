...
load bp_related\valsalva3_data.mat

figure;
subplot(311); plot(ECG); axis tight; %심전도
subplot(312); plot(PPG); axis tight; %ppg
subplot(313); plot(BP*100); axis tight; %혈압 
...
sbp_ =[824,1745,2606,3359,4524,5597,6694,7779,8894,10002,11093,12215,13319,14448,15535,16639,17632,18741,19855,20916,21992,22998,24090];
sbp=bp(sbp_');

% figure;
% plot(bp);hold on;axis tight;plot(sbp_, bp(sbp_),'ro');

%%
%DBP 구하기
dbp_=[692,1606,2467,3220,4385,5459,6566,7649,8763,9876,10973,12083,13193,14265,15365,16468,17506,18576,19685,20753,21821,22886,23925];
dbp=bp(dbp_');

% figure;
% plot(bp);hold on;plot(dbp_, bp(dbp_),'ro');

%sbp, dbp
figure;
plot(x, bp); hold on; plot(dbp_./fs,bp(dbp_), 'bo');axis tight;
plot(sbp_./fs,bp(sbp_), 'ro');axis tight;
%%
%ppg peak 찾기
ppg = data(:,2);
...
% p_ppg = filtfilt(b, a, p_ppg);

[p_pv, p_pi] = findpeaks(p_ppg);
p_pv=ppg(p_pi);
% p_pi = p_pi(p_pi~=12199 & p_pi~=15166 & p_pi~=18053);
% p_pv=ppg(p_pi);

% figure;
% plot(x, ppg);hold on;
% plot(p_pi./fs, p_pv, 'ro'); axis tight; xlabel('time');ylabel('ampl');

%ppg valley 찾기
v_avg_peak = -p_ppg;
[v_pv, v_pi] = findpeaks(v_avg_peak);
v_pv=ppg(v_pi);
% v_pi=v_pi(v_pi~=12170 & v_pi~=15133 & v_pi~=18016);
% v_pv=ppg(v_pi);
...
SBP3 = sbp;
DBP3 = dbp;
feature_HR3 = feature_HR;
feature_AREA3 = feature_AREA;
feature_HEIGHT3 = feature_HEIGHT;
feature_WIDRH3 = feature_WIDRH;
feature_STT3 = feature_STT;

save('blood_pressure\tr_3','SBP3','DBP3','feature_HR3','feature_AREA3','feature_HEIGHT3','feature_WIDRH3','feature_STT3');
